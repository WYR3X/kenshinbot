...
exports.run = (client, message, args) => {
 message.delete();
    message.guild.createChannel(`talep-${message.author.username}`, 'text').then(ch => {
        ...
        .setAuthor("Kenshin | Destek Sistemi")
        .setDescription("**Ekipimiz en kısa sürede sizinle beraber olacaktır ``k!kapat`` yazarak destek talebini iptal edebilirsin!**")
        .setFooter('Kenshin - Tüm Hakları Saklıdır.', client.user.avatarURL)
        .setTimestamp()
        ch.send(embed).catch()
        ch.send("@everyone")
        ch.send("@here")
        ch.awaitMessages((msg)=> {
            if (msg.content === "k!kapat") {
