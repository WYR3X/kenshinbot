...
module.exports.run = async (bot, message, args) => {
        ...
        if (oyndurum.has(message.channel.id)) return message.reply('<:kenshin_carpi:726170425779814413> **Kanal başına sadece bir adam asmaca oyunu meydana gelebilir.**');
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                await message.channel.send(stripIndents`
                    ${displayText === null ? '**Kenshin Adam Asmaca**!' : displayText ? '**Çok iyisin!**' : '**Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
                ...
                if (!guess.size) {
                    await message.channel.send('<:kenshin_carpi:726170425779814413> **Zamanın doldu!**');
                    break;
                }
            ...
            if (cevap.length === confirmation.length || tahmin) return message.reply(`<:kenshin_tik:726170369379139675> **Tebrikler kelimeyi buldun! ``${cevap}``**`);
            return message.channel.send(`<:kenshin_carpi:726170425779814413> Maalesef bilemedin kelime bu: **${cevap}**`);
        } catch (err) {
            oyndurum.delete(message.channel.id);
            return message.reply(`<:kenshin_carpi:726170425779814413> Olamaz! Bir Hata Verdi: \`${err.message}\``);
        }
