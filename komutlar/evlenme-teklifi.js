...
message.channel.send(`${mention}, ${message.author.username} bu kişi seninle evlenmek istiyor...
...
if(user.id !== mention.id) return;
  ...
  message.guild.members.get(message.author.id).setNickname(`${mention.username}'nin Çiçeği`)
  message.guild.members.get(mention.id).setNickname(`${message.author.username}'nin Papatyası`)
...
if(user.id !== mention.id) return;
  ...
  mention.send(`**Neden kabul etmedin.. 💔**`)
